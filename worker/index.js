...
const ADMIN_EMAIL = "namnh.ftu@gmail.com";
...
function secureHeaders(headers = new Headers()) {
	...
	headers.set(
		...
		[
			...
			"frame-src 'self' https://www.youtube.com https://www.youtube-nocookie.com https://www.facebook.com https://vr.justeasy.cn",
...
function response(body, init = {}) {
	return new Response(body, {
		...init,
		headers: secureHeaders(new Headers(init.headers)),
	});
}
	...
	return sanitizeHtml(html || "", {
		...
		allowedStyles: {
			...
			a: {
				...
				"background-color": [
					/^(transparent|#[0-9a-fA-F]{3,8}|rgba?\([\d\s,.%]+\))$/,
				],
...
function validateContent(content) {
	if (!content || typeof content !== "object" || Array.isArray(content))
		return "Invalid content payload";
...
async function renderPage(env, page, canonical, extra = {}, status = 200) {
	const [content, uploadsMap] = await Promise.all([
		getContent(env),
		getUploadMap(env),
	]);
	return htmlDocument(
		renderSite({
			...
			canonical,
			content,
			imageMap: { ...buildImageMap, ...uploadsMap },
			origin: SITE_ORIGIN,
...
function checkWriteOrigin(request) {
	...
	return Boolean(origin && url) && origin === url.origin;
}

function auditSecurity(request, event, detail = {}) {
	const url = requestUrl(request);
	console.log(
		JSON.stringify({
			type: "security",
...
async function requireAdmin(request, env) {
	if (!env.ACCESS_TEAM_DOMAIN || !env.ACCESS_AUD) {
		auditSecurity(request, "admin_auth_not_configured");
		return {
			response: json({ error: "Administrative access is not configured" }, 503),
		};
	}
	const token = request.headers.get("cf-access-jwt-assertion");
	if (!token) {
		auditSecurity(request, "admin_auth_missing");
	...
	try {
		...
		auditSecurity(request, "admin_auth_invalid", {
...
async function enforceRateLimit(request, binding, scope) {
	...
	const key = `${scope}:${Array.from(new Uint8Array(keyBytes).slice(0, 12), (byte) => byte.toString(16).padStart(2, "0")).join("")}`;
	const { success } = await binding.limit({ key });
	if (success) return null;
	auditSecurity(request, "rate_limited", { scope });
...
async function uploadImage(request, env) {
	...
	if (!/^image\/(png|jpe?g|webp|gif)$/i.test(file.type))
		return json({ error: "Chỉ chấp nhận ảnh PNG/JPG/WebP/GIF" }, 400);

	const info = await env.IMAGES.info(file.stream());
	if (
		!info.width ||
		!info.height ||
		info.width * info.height > MAX_UPLOAD_PIXELS
	)
		return json({ error: "Kích thước ảnh không hợp lệ hoặc quá lớn" }, 413);
	const fullWidth = info.width;
	const targets = [
		...new Set(WIDTHS.filter((width) => width < fullWidth).concat(fullWidth)),
	].sort((a, b) => a - b);
	...
	for (const width of targets) {
		...
		await env.UPLOADS.put(name, transformedResponse.body, {
...
async function handleRequest(request, env) {
	...
	const isAdminRoute =
		lowerPath === "/admin" ||
		lowerPath === "/api/content" ||
		lowerPath === "/api/upload";
	...
	if (url.hostname.toLowerCase() === "www.franzlift.vn")
		return redirect(SITE_ORIGIN + pathname + url.search);
	if (
		pathname.startsWith("/uploads/") &&
		(method === "GET" || method === "HEAD")
	)
		return serveUpload(env, pathname, method);
	...
	if (method === "GET" || method === "HEAD") {
		...
		if (asset.status !== 404) {
			...
			if (
				...
				pathname.startsWith("/assets-local/")
			)
	...
	if (REDIRECTS.has(lowerPath) && (method === "GET" || method === "HEAD"))
		return redirect(REDIRECTS.get(lowerPath));
	if (PAGE_ROUTES.has(lowerPath) && (method === "GET" || method === "HEAD")) {
		...
		return method === "HEAD"
			? response(null, { status: rendered.status, headers: rendered.headers })
			: rendered;
	}

	if (lowerPath === "/admin" && (method === "GET" || method === "HEAD")) {
		const content = await getContent(env);
		const rendered = htmlDocument(
			renderAdmin({ content, serializedContent: serializeForScript(content) }),
		);
		return method === "HEAD"
			? response(null, { headers: rendered.headers })
			: rendered;
	}

	if (lowerPath === "/robots.txt" && (method === "GET" || method === "HEAD")) {
		const body = `User-agent: *\nAllow: /\nDisallow: /admin\nDisallow: /api/\n\nSitemap: ${SITE_ORIGIN}/sitemap.xml\n`;
		return response(method === "HEAD" ? null : body, {
			headers: { "Content-Type": "text/plain; charset=utf-8" },
		});
	}
	...
	if (lowerPath === "/api/content" && method === "GET")
	...
	if (lowerPath === "/api/content" && method === "POST") {
		...
		if (declaredLength > MAX_CONTENT_BYTES)
			return json({ error: "Content payload too large" }, 413);
		...
		auditSecurity(request, "content_updated");
		return json({ ok: true, savedAt: new Date().toISOString() });
	}
	if (lowerPath === "/api/upload" && method === "POST") {
		const uploaded = await uploadImage(request, env);
		auditSecurity(
			request,
			uploaded.status < 400 ? "image_uploaded" : "image_upload_failed",
	...
	if (productMatch && (method === "GET" || method === "HEAD")) {
		const content = await getContent(env);
		const index = (content.productItems || []).findIndex(
			(item) => item.slug === productMatch[1],
		);
		const rendered =
			index === -1
	...
	if (postMatch && (method === "GET" || method === "HEAD")) {
		const content = await getContent(env);
		const index = (content.postItems || []).findIndex(
			(item) => item.slug === postMatch[1],
		);
		const rendered =
			index === -1
				? await renderPage(env, "notfound", pathname, {}, 404)
				: await renderPage(env, "postDetail", `/tin-tuc/${postMatch[1]}`, {
						postIndex: index,
					});
		return method === "HEAD"
			? response(null, { status: rendered.status, headers: rendered.headers })
	...
	if (
		/^\/(youjiasG1|youjiatG2|yijia|Rli|sx)\/\d+$/i.test(normalized) &&
		(method === "GET" || method === "HEAD")
	) {
		const content = await getContent(env);
		const item = (content.productItems || []).find(
			(product) => product.legacyPath === normalized,
		);
